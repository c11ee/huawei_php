...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('product_spu', function (Blueprint $table) {
            ...
            $table->string('product_description', 255)->nullable()->comment('产品描述');
            $table->bigInteger('brand_id')->comment('品牌ID');
            $table->json('slider_images')->nullable()->comment('轮播图');
            $table->string('video_url', 255)->nullable()->comment('视频URL');
            $table->string('video_cover_url', 255)->nullable()->comment('视频封面URL');
            $table->integer('spec_template_id')->comment('规格模板ID');
            $table->bigInteger('default_sku_id')->comment('默认SKU ID');
            $table->decimal('default_sku_sale_price', 12, 2)->comment('默认SKU销售价格');
            $table->integer('default_sku_stock')->comment('默认SKU库存');
            $table->string('default_sku_image_url', 255)->nullable()->comment('默认SKU主图');
